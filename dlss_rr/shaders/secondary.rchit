...
vec3 sampleLights(in HitState state, inout uint seed, out vec3 dirToLight, out float lightPdf)
{
  ...
  else
  {
    ...
    vec4 radiance_pdf = environmentSample(hdrTexture, randVal, dirToLight);
...
ShadingResult shading(in PbrMaterial pbrMat, in HitState hit)
{
  ...
  if(lightValid)
  {
    ...
    if(evalData.pdf > 0.0)
    {
      ...
      traceRayEXT(topLevelAS, ray_flag, 0xFF, SBTOFFSET_SECONDARY, 0, MISSINDEX_SECONDARY, shadowRayOrigin, 0.001,
                  dirToLight, DLSS_INF_DISTANCE, PAYLOAD_SECONDARY);
